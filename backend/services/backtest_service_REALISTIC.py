                    ...
                    if current_position:
                        ...
                        if not exit_price:
                            for lv in levels:
                                ...
                                # ANTI-PING-PONG: Ignore the level that triggered the current entry
                                if lv['p'] == entry_lv_p:
                                    continue
                                ...
                                if side == "BUY":
                                    ...
                                    if test_p_down < (lv['p'] - b) and prev_c_ref >= (lv['p'] - b):
                                        # REALISTIC FIX: Use close_p for Candle Close mode
                                        exit_price, reason = (close_p if trigger_mode == 'CANDLE_CLOSE' else lv['p'] - b), f"TRAP_{lv['n']}"
                                        break
