...
def load_config(page):
    """Load config using client_storage (works on mobile)"""
    default = {"api_key": "", "client_id": "", "watchlist": [], "telegram_bot_token": "", "telegram_chat_id": ""}
    try:
        # Try client_storage first (mobile-friendly)
        if page and page.client_storage:
            api_key = page.client_storage.get("api_key") or ""
            client_id = page.client_storage.get("client_id") or ""
            watchlist_json = page.client_storage.get("watchlist") or "[]"
            telegram_token = page.client_storage.get("telegram_bot_token") or ""
            telegram_chat = page.client_storage.get("telegram_chat_id") or ""
            return {
                "api_key": api_key,
                "client_id": client_id,
                "watchlist": json.loads(watchlist_json) if watchlist_json else [],
                "telegram_bot_token": telegram_token,
                "telegram_chat_id": telegram_chat
            }
        # Fallback to file for desktop
...
def save_config(page=None):
    """Save config using client_storage (works on mobile)"""
    clean_watchlist = []
    ...
    # Save to client_storage (mobile + desktop)
    if page and page.client_storage:
        try:
            page.client_storage.set("api_key", state.api_key)
            page.client_storage.set("client_id", state.client_id)
            page.client_storage.set("watchlist", json.dumps(clean_watchlist))
            page.client_storage.set("telegram_bot_token", state.telegram_bot_token)
            page.client_storage.set("telegram_chat_id", state.telegram_chat_id)
            print(f"Saved {len(clean_watchlist)} symbols to storage")
        except Exception as e:
            print(f"Client storage save error: {e}")
    
    # Also save to file for desktop backward compatibility
    try:
        data = {
        ...
        print(f"File save error (desktop only): {e}")
    ...
    def get_watchlist_view():
        ...
        controls.append(ft.Container(
            content=ft.Row([
                ...
                ft.Dropdown(label="Sort", value=state.sort_by, dense=True, width=120,
                           on_change=on_sort_change, border_color="#2D3748",
                           options=[
                               ft.dropdown.Option("none", "Default"),
                               ft.dropdown.Option("sym_az", "A-Z"),
                               ft.dropdown.Option("sym_za", "Z-A"),
                               ft.dropdown.Option("price_low", "Low"),
                               ft.dropdown.Option("price_high", "High"),
                           ]),
                ft.IconButton("clear", on_click=clear_filters, icon_size=20, icon_color="#667EEA")
            ]),
            padding=8, bgcolor="#222844", border_radius=8, border=ft.border.all(1, "#2D3748")
        ))
