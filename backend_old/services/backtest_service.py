...
class BacktestService:
    ...
    def run_backtest(self, smart_api, symbol: str, token: str, exch: str, start_date: str, end_date: str, strategy_config: Dict):
        ...
        target_val = strategy_config.get('target', 0)
        target_type = strategy_config.get('target_type', 'POINTS') 
        sl_val = strategy_config.get('stop_loss', 0)
        trailing_val = strategy_config.get('trailing_sl', 0)
                    ...
                    if current_position:
                        ...
                        if target_val:
                            if target_type == "POINTS":
                                ...
                                # PnL = (price - entry) * qty
                                # Required price = (TargetAmount / qty) + entry
                                ppts = target_val / quantity
                                if side == "BUY" and high_p >= (entry_p + ppts): exit_price, reason = entry_p + ppts, "TARGET_AMT"
                                elif side == "SELL" and low_p <= (entry_p - ppts): exit_price, reason = entry_p - ppts, "TARGET_AMT"

                        # B. Stop Loss Monitoring
                        ...
                        # C. Trailing SL (Simplified)
                        trailing_val = strategy_config.get('trailing_sl', 0)
                        if not exit_price and trailing_val and current_position.get('highest_high'):
                            # Implementation of trailing SL logic would go here
                            pass

                        # D. Strategy Levels (SAR)
                        if not exit_price:
                            for lv in levels:
                                b = lv['p'] * buffer_pct
                                if side == "BUY":
                        ...
                        if exit_price:
                            ...
                            exit_price = round(exit_price * 20) / 20.0
                            ...
                            # If it was an EXIT (TGT/SL), keep flat until next breakout. 
                            # If it was a REVERSE (TRAP/REJ), flip position.
